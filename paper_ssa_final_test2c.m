...
% Only changes the values of t and p from paper_ssa_final_test2a.m
...
p = 199;	% max number of Krylov iterations
...
title(sprintf('$%d$ matrices, $m = %d$, $k = %d$, $\\textrm{cond} < 10^{%d}$', length(ids), p+1, t, floor(log10(condbound))));
xlabel('$\theta$'), ylabel('basis size performance'); 
fname = sprintf('ssa_plots/test2bdim_%d_m%d_s%d_k%d_cnd%d',length(ids),p+1,s,t,floor(log10(condbound)));
...
grid on;
grid minor;
fname = sprintf('ssa_plots/test2bdim_%d_m%d_s%d_k%d_cnd%d_size',length(ids),p+1,s,t,floor(log10(condbound)));
ylim([-5, p+6]);
% mypdf(fname,0.66,1.8)
mypdf(fname,0.66,0.85)
