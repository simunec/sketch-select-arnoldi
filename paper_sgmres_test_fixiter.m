...
% All methods are run for the same number of iterations on each test problem
...
for testidx = 1:length(ids_selected)
	...
	% fname = sprintf('ssa_plots/test_sgmres_fixiter_t%d-m%d-singlemat%d.png', t, mmax, idj);
...
fname = sprintf('ssa_plots/test_sgmres_fixiter_t%d-m%d-r%d', t, mmax, abs(floor(log10(restol))));
